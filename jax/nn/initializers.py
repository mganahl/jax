...
def zeros(key, shape, dtype=np.float32):
  return lax.sample(lax.tie_in(key, np.zeros(shape, dtype)))

def ones(key, shape, dtype=np.float32):
  return lax.sample(lax.tie_in(key, np.ones(shape, dtype)))

def uniform(scale=1e-2):
  def init(key, shape, dtype=np.float32):
    return lax.sample(random.uniform(key, shape, dtype) * scale)
  return init

def normal(stddev=1e-2):
  def init(key, shape, dtype=np.float32):
    return lax.sample(random.normal(key, shape, dtype) * stddev)
  return init
...
def variance_scaling(scale, mode, distribution, in_axis=-2, out_axis=-1):
  def init(key, shape, dtype=np.float32):
    ...
    if distribution == "truncated_normal":
      ...
      return lax.sample(random.truncated_normal(key, -2, 2, shape, dtype) * stddev)
    elif distribution == "normal":
      return lax.sample(random.normal(key, shape, dtype) * np.sqrt(variance))
    elif distribution == "uniform":
      return lax.sample(random.uniform(key, shape, dtype, -1) * onp.sqrt(3 * variance))
    else:
